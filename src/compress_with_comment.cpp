...
const char *LZSS_HEADER_SYMBOL = "LZSS";
...
// ==================== both ===========================
const char *BOTH_HEADER_SYMBOL = "BTLH";
...
void dlzss(byte *rbuf, size len, buf *d)
{
    ...
    for(int i=0; i<4; ++i)
    {
        ...
        {
            printf("unvalid file: incorrect symbol(%4s)\n", symbol);
            exit(-1);
        }
    }
...
void compress_both(const char *src, const char *dst)
{
    ...
    fwrite(BOTH_HEADER_SYMBOL, 1, 4, f);
...
void decompress_both(const char *src, const char *dst)
{
    ...
    for(int i=0; i<4; ++i)
    {
        if(symbol[i] != BOTH_HEADER_SYMBOL[i])
        {
            printf("unvalid file: incorrect symbol(%4s)\n", symbol);
            exit(-1);
        }
